...
**加測「模擬演練」**（2026-07-03 第二次 Fable session 驗證有效，一次就抓到 4 個真卡點）：
另派一個 fresh-context sonnet agent，給它 1~2 個擬真使用者任務（一個一般實作、
一個含歧義/不可逆風險），要求它「只演練不改檔」：照 CLAUDE.md 走一遍，
回報讀檔順序、行動計畫、完成判準、會不會停下問，以及**卡點清單**
（哪句制度文字兩讀皆可／找不到指引／互相矛盾，引用檔名＋節）。
prompt 結尾必加「誠實優先：想不起來要看的規則就直說」，且不可透露你期望的答案。
靜態審查抓「文字錯誤」，模擬演練抓「弱模型實際會誤讀什麼」——兩者互補，都要做。
