...
例外：預期改動 <10 行，且「定位」只需 codebase-map + 1~2 次 Grep——這種直接做，別為省而虧。
明確判準：**動手前如果需要 Read 超過 1 個沒讀過的檔、或超過 200 行，就派工**；
只是 Grep 幾下確認行號不算「搜尋」，不必為此派 agent。
...
## 7. 費用比例（2026-07-03 查證，來源：claude-api skill 官方定價表）

API 定價（input/output，每百萬 token）：haiku 4.5 = $1/$5，sonnet 5 = $3/$15，
opus 4.8 = $5/$25，fable 5 = $10/$50。換算相對比例：**haiku 1x、sonnet 3x、opus 5x、fable 10x**。

由此推出的調度含義（比舊版「opus=15x」的假設寬鬆）：
- opus 只比 sonnet 貴約 1.7 倍——**該升級就升級，不要為省這點差價多浪費一輪重試**
  （一輪失敗重試燒掉的 token 通常超過那 1.7 倍差價）。
- 真正的成本主力是「餵進去的 context 量」與思考深度，不是模型單價。
  省錢的重點永遠是：交辦 prompt 精準、不整檔讀、回報短——而不是死守便宜模型。
- haiku→sonnet 差 3 倍，所以機械批次工作仍優先 haiku。
- 升降級的「紀律」照第 5 節不變：先帶失敗軌跡再升級，理由是品質（讓 opus 不重蹈覆轍），
  不只是省錢。
- 注意：訂閱方案的額度計算方式與 API 定價未必相同，此比例當量級參考即可。
